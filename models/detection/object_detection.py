...
from models.detection.relation_net import RelationNet
from configs.data_config import NYU40CLASSES,Config
...
class Bdb3DNet(nn.Module):
    ...
    def forward(self, x, size_cls, g_features, split, rel_pair_counts,box_feat):
        '''
        ...
        a_features = torch.cat([a_features,box_feat],dim=1)
        a_features = self.conv_res(a_features)
        ...
        #print(a_r_features.shape,size_cls.shape,box_feat.shape)
        a_r_features_cat = torch.cat([a_r_features, size_cls,box_feat], 1)

        # branch to predict the size
        #print(a_r_features_cat.shape)
