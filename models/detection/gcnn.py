...
class GCNN(nn.Module):
    def __init__(self, cfg, optim_spec=None):
        ...
        '''configs and params'''
        self.cfg = cfg
        self.lo_features = cfg['model']['output_adjust']['lo_features']
        self.obj_features = cfg['model']['output_adjust']['obj_features']
        self.rel_features = cfg['model']['output_adjust']['rel_features']
        feature_dim = cfg['model']['output_adjust']['feature_dim']
        self.feat_update_step = cfg['model']['output_adjust']['feat_update_step']
        self.res_output = cfg['model']['output_adjust'].get('res_output', False)
        self.feat_update_group = cfg['model']['output_adjust'].get('feat_update_group', 1)
        self.res_group = cfg['model']['output_adjust'].get('res_group', False)

        self.feature_length = {
            'size_cls': 9, 'cls_codes': 9,
            ...
            'ldif_afeature': cfg['model']['bottleneck_size'],
            'analytic_code': cfg['model']['analytic_code_len'],
            'blob_center': (cfg['model']['element_count']
                            + cfg['model']['sym_element_count']) * 3,
            'ldif_phy': (cfg['model']['element_count']
                         + cfg['model']['sym_element_count']) // 2,
            'structured_implicit_vector': cfg['model']['structured_implicit_vector_len']
        }
    ...
    def _get_bdb3D_form(self, data):
        ...
        cam_R_out,_ = get_rotation_matix_result(bins_tensor,
    ...
    def _get_object_features(self, data, type):
        ...
        for k in keys:
            if k in ['size_cls', 'cls_codes', 'size_reg_result', 'ori_reg_result', 'ori_cls_result',
                     'centroid_reg_result', 'centroid_cls_result', 'offset_2D_result',"occ_cube",
                ...
                for center_ind, (start, end) in zip(center_inds.long(), data['split'].long()):
            ...
            if type == 'obj' or k in ('g_features', 'ldif_phy'):
                ...
                features_rel = []
                for start, end in data['split'].long():
    ...
    def forward(self, output):
        ...
        #print(x_obj.shape,x_pred.shape)
        ...
        rel_pair = output['rel_pair_counts'].long()
        for lo_index, (start, end) in enumerate(output['split'].long()):
