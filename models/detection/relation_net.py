...
class RelationNet(nn.Module):
    ...
    def forward(self, a_features, g_features, split, rel_pair_counts):
        ...
        for interval_idx, interval in enumerate(split):
            sample_k_features = k_features[:, interval[0].long():interval[1].long(), :]
            sample_q_features = q_features[:, interval[0].long():interval[1].long(), :]

            sample_a_weights = torch.div(torch.bmm(sample_k_features, sample_q_features.transpose(1, 2)), math.sqrt(rel_cfg.d_k))

            sample_g_weights = g_weights[:, rel_pair_counts[interval_idx].long():rel_pair_counts[interval_idx + 1].long()]
            sample_g_weights = sample_g_weights.view(sample_g_weights.size(0), (interval[1]-interval[0]).long(), (interval[1]-interval[0]).long())
            ...
            sample_v_features = v_features[:, interval[0].long():interval[1].long(), :]
